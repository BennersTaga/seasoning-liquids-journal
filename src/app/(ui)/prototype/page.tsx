...
function deriveDataFromMasters(masters?: Masters) {
  ...
  const uses =
    masters?.uses?.map(u => ({
      code: u.use_code,
      name: u.use_name,
      type: u.use_type as "fissule" | "oem",
    })) ?? [];
...
export default function App() {
  ...
  const { factories, storageByFactory, flavors, oemList, uses, allowedByUse } = useMemo(
    () => deriveDataFromMasters(mastersData ?? EMPTY_MASTERS),
    [mastersData],
  );
  ...
  return (
    <div className="min-h-screen bg-orange-50 p-6 mx-auto max-w-7xl space-y-6">
      ...
      <footer className="text-xs text-center text-muted-foreground opacity-70">
        GAS 連携バージョン
      </footer>
    </div>
  );
...
function Office({
  ...
  const derivedUseType: "fissule" | "oem" =
    selectedUse?.type === "oem" ? "oem" : "fissule";
  ...
  const openOrders = useMemo(
    () => orderCards.filter(order => !order.archived),
    [orderCards],
  );
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Select value={factory} onValueChange={setFactory}>
              ...
              >
                <SelectValue
                  placeholder={mastersLoading ? "読み込み中..." : "未設定"}
                />
              </SelectTrigger>
              <SelectContent>
                {factories.length
          ...
          {derivedUseType === "fissule" ? (
            <div>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                必要量:{" "}
                {formatGram(
                  packs * (findFlavor(flavor)?.packToGram ?? 0),
                )}
              </div>
            </div>
          ) : (
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="font-medium">
                  ...
                  <Badge variant="secondary" className="ml-2">
                    {
                      factories.find(
                        f => f.code === order.factoryCode,
                      )?.name || order.factoryCode
                    }
                  </Badge>
                </div>
                ...
                return (
                  <div key={idx} className="text-sm grid gap-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <Field label="用途">
                        {(() => {
                          const label = ln.useCode
                            ? purposeLabelByCode[ln.useCode] ??
                              ln.useCode
                            : undefined;
                          const typeLabel =
                            ln.useType === "oem" ? "OEM" : "製品";
                          return label
                            ? `${label}（${typeLabel}）`
                            : typeLabel;
                        })()}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      >
                        {ln.useType === "fissule"
                          ? formatPacks(
                              ln.packsRemaining ?? ln.packs ?? 0,
                            )
                          : ln.oemPartner ?? "-"}
                      </Field>
                      <Field label="必要量">
                        <span className="font-semibold">
                          {formatGram(ln.requiredGrams)}
                        </span>
                      </Field>
                    </div>
                  </div>
    ...
    async (order: OrderCard, report: MadeReport) => {
      ...
      const leftoverPayload =
        report.leftover && report.leftover.grams > 0
          ? {
              location: report.leftover.location,
              grams: report.leftover.grams,
            }
          : null;
...
function QtyCell({
  ...
  const gramText = Number.isFinite(grams as number)
    ? formatGram(grams ?? 0)
    : "-";
  return (
...
type FloorTableProps = {
  ...
  findFlavor: (flavorId: string) => FlavorWithRecipe;
  storageByFactory: Record<string, string[]>;
  mastersLoading: boolean;
  keepBusy: boolean;
  reportBusy: boolean;
    ...
    () =>
      (orders ?? [])
        ...
        .map(row => {
          ...
          const storageEntry = storageMap.get(
            `${row.lot_id}-${row.flavor_id}`,
          );
          const isOem =
            (row.use_type ?? "").toLowerCase() === "oem";

          const shouldPacks = isOem
            ? undefined
          ...
          const madePacks = Number.isFinite(row.made_packs as number)
            ...
            : Number.isFinite(row.packs as number) &&
                Number.isFinite(row.packs_remaining as number)
              ? Math.max(
                  0,
                  (row.packs as number) -
                    (row.packs_remaining as number),
                )
              : 0;
          ...
          const status: StatusType = (() => {
            ...
            if (
              shouldPacks !== undefined &&
              madePacks >= shouldPacks &&
              row.archived
            )
              return "全量使用";
            if (shouldPacks !== undefined && madePacks >= shouldPacks)
              return "製造完了";
            if (
              madePacks > 0 &&
              (shouldPacks === undefined || madePacks < shouldPacks)
            )
              return "製造中";
            return "指示";
          })();
          ...
          const madeGrams =
            packToGram > 0 && madePacks > 0
              ? madePacks * packToGram
              : undefined;

          const hasPacksEquiv =
            typeof storageEntry?.packs_equiv === "number" &&
            Number.isFinite(storageEntry.packs_equiv);
...
function FloorTableDataRow({
  ...
  const [open, setOpen] = useState<
    null | "keep" | "made" | "skip" | "choice" | "split"
  >(null);
  const line = card?.lines?.[0];

  const remainingPacks = line
    ? Math.max(0, line.packsRemaining ?? line.packs ?? 0)
    : 0;
  const canSplit = line
    ? line.useType === "fissule" && (line.packs ?? 0) > 0
    : false;

  const packsLabel =
    shouldPacks !== undefined
      ? `${formatPacks(shouldPacks)}パック分`
      : "OEM";
  const madePacksLabel =
    shouldPacks !== undefined
      ? `${formatPacks(madePacks)}パック分`
      : "-";
  const leftoverPacksLabel =
    leftoverPacks !== undefined
      ? `${formatPacks(leftoverPacks)}パック分`
      : undefined;
      ...
      >
        ...
        <td className="px-4 py-3 text-sm text-slate-700 text-right">
          <QtyCell
            grams={
              madeGrams ??
              (packToGram > 0 ? madePacks * packToGram : undefined)
            }
            packsLabel={madePacksLabel}
          />
        </td>
        <td className="px-4 py-3 text-sm text-slate-700 text-right">
          <QtyCell
            grams={leftoverGrams}
            packsLabel={leftoverPacksLabel}
          />
        </td>
        <td
          className="px-4 py-3 text-sm text-slate-700"
          title={(storageEntry?.locations || []).join(" / ")}
        >
          {storageEntry?.locations?.length
...
function defaultOrderFromRow(row: OrderRow): OrderCard {
  const packsNum = Number(row.packs);
  const packsRemNum = Number(row.packs_remaining);
  const requiredGramsNum = Number(row.required_grams);
    ...
    lines: [
      {
        flavorId: row.flavor_id,
        packs: Number.isFinite(packsNum) ? packsNum : 0,
        packsRemaining: Number.isFinite(packsRemNum)
          ? packsRemNum
          : Number.isFinite(packsNum)
            ? packsNum
            : 0,
        requiredGrams: Number.isFinite(requiredGramsNum)
          ? requiredGramsNum
          : 0,
        useType:
          (row.use_type ?? "").toLowerCase() === "oem"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              ...
              <Select value={loc} onValueChange={setLoc}>
                <SelectTrigger
                  disabled={mastersLoading || locations.length === 0}
                >
                  <SelectValue
                    placeholder={mastersLoading ? "読み込み中..." : "未設定"}
                  />
...
function MadeDialog2({
  ...
  const tooMuch =
    showPackInput && packsMade > Math.max(0, remaining);
  ...
  useEffect(() => {
    if (!open) return;
    const sum = flavor.recipe.reduce(
      (total, r) => total + (r.qty || 0),
      0,
    );
    ...
    flavor.recipe.forEach(r => {
      ...
      const value =
        sum > 0 ? Math.round(grams * ((r.qty || 0) / sum)) : 0;
      exp[key] = value;
